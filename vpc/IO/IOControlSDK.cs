    ...
    class IOHIKCIO : IOHdl
    {
        ...
        public bool Init()
        {
            ...
            if (CIOControllerSDK.MV_OK != nRet || IntPtr.Zero == handle)
                throw new Exception("CIOController IO初始化错误： Creating the handle failed.Err code:" + Convert.ToString(nRet, 16));
            m_stSerial.strComName = "Com3";
            nRet = CIOControllerSDK.MV_IO_Open_CS(handle, ref m_stSerial);
            if (CIOControllerSDK.MV_OK != nRet)
                throw new Exception($"CIOController IO初始化错误：{m_stSerial.strComName}打开失败_{nRet:X}");
            CIOControllerSDK.MV_IO_SET_OUTPUT stOutput = new CIOControllerSDK.MV_IO_SET_OUTPUT();
            stOutput.nPort = (UInt32)(CIOControllerSDK.MV_IO_PORT_NUMBER.MV_IO_PORT_1);
            stOutput.nType = (UInt32)(CIOControllerSDK.MV_IO_PATTERN_OUT.MV_IO_PATTERN_SINGLE);
            stOutput.nDurationTime = IO.PluseWidth;
            stOutput.nValidLevel = (UInt32)(CIOControllerSDK.MV_IO_LEVEL.MV_IO_LEVEL_LOW);
            nRet = CIOControllerSDK.MV_IO_SetOutput_CS(handle, ref stOutput);
            if (CIOControllerSDK.MV_OK != nRet)
                throw new Exception($"CIOController 输出模式设定错误1：{nRet:X}");

            stOutput = new CIOControllerSDK.MV_IO_SET_OUTPUT();
            stOutput.nPort = (UInt32)(CIOControllerSDK.MV_IO_PORT_NUMBER.MV_IO_PORT_2);
            stOutput.nType = (UInt32)(CIOControllerSDK.MV_IO_PATTERN_OUT.MV_IO_PATTERN_SINGLE);
            stOutput.nDurationTime = IO.PluseWidth;
            stOutput.nValidLevel = (UInt32)(CIOControllerSDK.MV_IO_LEVEL.MV_IO_LEVEL_LOW);
            nRet = CIOControllerSDK.MV_IO_SetOutput_CS(handle, ref stOutput);
            if (CIOControllerSDK.MV_OK != nRet)
                throw new Exception($"CIOController 输出模式设定错误2：{nRet:X}");

            return true;
        }
        public bool SetOutput(int portid)
        {
            if (portid >= 0 && portid < pn.Length)
            {

                CIOControllerSDK.MV_IO_OUTPUT_ENABLE stParam = new CIOControllerSDK.MV_IO_OUTPUT_ENABLE();
                stParam.nPortNumber = (ushort)pn[portid];
                ...
                var nRet = CIOControllerSDK.MV_IO_SetOutputEnable_CS(handle, ref stParam);

                if (CIOControllerSDK.MV_OK != nRet)
                {
                    Program.ErrHdl($" Set portid {portid} fail: Ret = {nRet:X}");
                    return false;
                }
                else
                    return true;
                ...
                Program.ErrHdl("输出portid超限：" + portid);
            return false;
        }
        ...
        public void Dispose()
        {
            ...
            {
                CIOControllerSDK.MV_IO_Close_CS(handle);
                CIOControllerSDK.MV_IO_DestroyHandle_CS(handle);
                handle = IntPtr.Zero;
            }
        }

        CIOControllerSDK.MV_IO_PORT_NUMBER[] pn = new CIOControllerSDK.MV_IO_PORT_NUMBER[]
        {
            CIOControllerSDK.MV_IO_PORT_NUMBER.MV_IO_PORT_1
            ,CIOControllerSDK.MV_IO_PORT_NUMBER.MV_IO_PORT_2
            ,CIOControllerSDK.MV_IO_PORT_NUMBER.MV_IO_PORT_3
            ,CIOControllerSDK.MV_IO_PORT_NUMBER.MV_IO_PORT_4
            ,CIOControllerSDK.MV_IO_PORT_NUMBER.MV_IO_PORT_5
            ,CIOControllerSDK.MV_IO_PORT_NUMBER.MV_IO_PORT_6
            ,CIOControllerSDK.MV_IO_PORT_NUMBER.MV_IO_PORT_7
            ,CIOControllerSDK.MV_IO_PORT_NUMBER.MV_IO_PORT_8
        };
